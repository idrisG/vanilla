...
function generateSpinner() {
    if (numberPointsInput && radiusSpinnerInput) {
        const numberPoints = (<HTMLInputElement>numberPointsInput).value;
        const radiusSpinner = (<HTMLInputElement>radiusSpinnerInput).value;
        if (
            preview &&
            code &&
...
function generateCode(number: number, radius: number) {
    var angle;
    var translateX;
    var translateY;
