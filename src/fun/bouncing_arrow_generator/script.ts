...
function generateBouncingArrow() {
    ...
    if (!!widthInput && !!lengthInput && !!radiusInput && !!directionInput) {
        const w = (<HTMLInputElement>widthInput).value;
        const l = (<HTMLInputElement>lengthInput).value;
        const r = (<HTMLInputElement>radiusInput).value;
        const d = (<HTMLInputElement>directionInput).value;
        if (!!w && !!l && !!r && !!d) {
            const inputValid =
                checkInputs([w, l, r]) && !!d && d.match(numberRegexAccepts0);
            if (inputValid) {
